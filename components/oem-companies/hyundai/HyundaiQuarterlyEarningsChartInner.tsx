...
import type { HyundaiQuarterlyEarningsPoint } from '@/lib/types';
...
/** 매출 막대 위 데이터 라벨 — bn → 조원 (Tooltip과 일관). NULL은 빈 문자열. */
...
/** 분기 매출 + 영업이익률 듀얼 축 ComposedChart.
 *  bar: revenue_krw_bn (왼쪽 Y축, 십억원 → 차트는 조원 표기)
 *  line: operating_margin_pct (오른쪽 Y축, %)
 *  recharts는 NULL 데이터에 대해 자동으로 점 생략. */
export default function HyundaiQuarterlyEarningsChartInner({ data }: Props) {
      ...
      <ComposedChart data={data} margin={{ top: 32, right: 24, bottom: 10, left: 10 }}>
        ...
        <Tooltip
          ...
          formatter={(value, name) => {
            ...
            if (name === '매출') {
              const tn = v / 1000; // bn → trillion KRW
              return [`${v.toLocaleString('ko-KR')} 십억원 (≈${tn.toFixed(1)}조원)`, '매출'];
            }
            if (name === '영업이익률') return [`${v.toFixed(2)}%`, '영업이익률'];
