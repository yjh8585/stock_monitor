...
/**
 * 게시글 상세 본문 — `/reports/[id]` 와 `/humanoid/reports/[id]` 가 함께 쓴다.
 *
 * 로봇 글은 휴머노이드 탭 안에서 읽어야 해서 라우트가 둘이 됐다(사용자 결정 2026-09-11).
 * 🔴 화면을 복제하지 말 것 — 사외비 게이트·삭제 권한·상태 감시가 **양쪽에 다 있어야** 하는데
 *    복제하면 한쪽만 고쳐지고 다른 쪽은 조용히 낡는다. 다른 것은 「목록으로」 링크뿐이다.
 */
...
interface Props {
  /** 라우트 파라미터의 id (문자열 그대로 받는다 — 숫자 판정은 여기서 한다) */
  id: string;
  /** 「목록」 버튼이 돌아갈 곳 */
  backHref: string;
}

export async function PostDetail({ id, backHref }: Props) {
