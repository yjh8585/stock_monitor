...
6. 원본 보고서를 옮길 때는 **원본의 표·이미지를 그대로 보존**(다시 그리거나 mermaid로 재구성 금지). 비교가 필요하면 새 GFM 표 작성 가능.
   - 🔴 **유료 사이트 원문은 「받아졌다」를 HTTP 200 으로 판정하지 말 것.** MarkLines 는 로그인이 안 돼도 **200 에 34만 자**를 돌려주는데 본문이 유료 벽이라 **`<table>` 이 0개**다. 표가 본체인 보고서에서 이걸 놓치면 표 없는 요약이 그럴듯하게 나와 더 안 걸린다. 판별은 `'class="register-paid-box"' in html` 과 `<table>` 개수 — 쿠키가 두 곳에 있고 껍데기가 이기는 사고까지 → [`docs/oem-collection.md`](./docs/oem-collection.md) 「MarkLines 쿠키가 «두 곳»에 있고」
   - ⚠️ **웹 클리퍼(옵시디언 등) 저장본은 HTML 속성에 실린 데이터를 잃는다** — `bgcolor` 로 칠해 둔 값이 빈칸이 되어 의미가 뒤집힐 수 있다(「조사 중」 → 「공급 없음」). 표가 중요하면 클리핑 말고 원문을 받는다.
   - **초광폭 행렬(20열 초과)은 그대로 옮기면 못 읽는다.** 축을 뒤집어 다시 짜거나(정보 손실 없음) 정말 시각적인 것만 PNG 로 올린다 — 긴 표를 이미지로 만들면 검색·복사·모바일 줄바꿈이 죽는다.
...
### 3-B. 외화 수치는 원화 환산을 괄호로 병기한다 🔴 (사용자 지시 2026-09-15)

> **사용자 지시 원문** — "마크라인 출처인 보고서를 보고 있어. 마크라인 뿐만 아니라 모든 보고서에서
> 원화 아닌 통화로 수치가 나오면 괄호하고 원화로 환산해서 써줘. 환율을 그 당시 환율 기준으로."
>
> **같은 날 추가 지시** — "환율을 어렵게 생각하지 말고 보고서 작성시점의 환율 그냥 집어넣어.
> 복잡하게 만들지 마 어차피 환산값인데. 과거 계열은 소급하지 말고 앞으로 적용해"

**적용 범위는 보고서 전부다.** 출처가 마크라인이든 유튜브든 PDF든, 자동 요약이든 직접 작성이든
가리지 않는다. 달러·유로·엔·위안·파운드로 적힌 **금액**이 나오면 그 자리에 원화를 병기한다.
**기존 글은 소급하지 않는다** — 앞으로 쓰는 글에만 적용한다.

🔴 **환율은 「보고서 작성 시점」 하나만 쓴다.** 수치마다 회계연도를 따져 다른 환율을 찾지 말 것 —
**어차피 환산값**이고, 그렇게 만들면 쓰는 사람도 읽는 사람도 피곤해진다(추가 지시 2026-09-15).

- **환율 출처는 DB `exchange_rates_live`**(현재값 · USD/EUR/GBP/JPY/HKD/CNY).
  헬퍼는 `scripts/lib/fx.py`의 `krw_rate_on(base)` — 애드혹 SQL 을 다시 짜지 말 것.
- **적용 환율은 글 안에 한 번만 각주로** 남긴다(각 수치마다 적지 않는다). 그래야 다음 사람이
  검산할 수 있고, 환율이 하나라 각주도 한 줄이면 끝난다.
- **범위값**(「수억 유로」·「수천만 달러」)은 환산할 값이 없다. **단위 하나만 기준으로 병기**한다 —
  `**수억 유로**(1억 유로 ≈ 1,614억원)`.

**표기 형식**

```markdown
전사 매출 **328.37억 유로**(약 51.0조원, 2025년 9월 결산)
모빌리티 솔루션 매출은 **558억 유로**(약 89.5조원)로 전사의 약 **62%**
멕시코 몬테레이에 **2억 달러**(약 2,734억원) 규모 첨단 기술 센터

_원화는 작성 시점(2026-09-15) 환율 적용 — 1유로 1,602원 · 1달러 1,420원._
```

- 자릿수는 **10조 이상 소수 첫째 자리 · 1~10조 소수 둘째 자리 · 1조 미만 억원 정수**.
  (실제로 쓰는 값이 무엇인지 읽는 사람이 바로 잡히게 — 유효숫자를 과하게 남기지 않는다.)
- 원 수치 뒤 괄호에 이미 기준일이 있으면 **한 괄호 안에 쉼표로 합친다** — 괄호를 두 번 열지 않는다.
- 🔴 **표는 「원화 환산」 열을 더하거나, 표가 넓으면 바로 아래에 별도 환산표를 둔다.**
  통화가 섞인 비교표는 환산 없이는 애초에 비교가 안 된다(유로 기업과 달러 기업을 나란히 놓은 표).

**검사** — `python scripts/verify_report_krw.py --run <원고>` 또는 `--post <id>`. EXIT=0 이 정상.
...
- **단독 줄 `<br>` 지양**: 빈 줄로 문단을 나눈다. 단독 `<br>` 뒤에 빈 줄이 없으면 다음 문단이 HTML 블록으로 흡수돼 마크다운 전체가 무력화된다(전처리가 제거하지만 애초에 쓰지 않는 게 깔끔). **표 셀 안 줄바꿈**용 `<br>`는 사용 가능하다 — 🔴 단 이것은 **2026-09-11 에 렌더러를 고친 뒤부터**다. 그전에는 `rehype-raw` 가 없어 `<br>` 가 화면에 **글자 그대로** 찍혔다(리포트 115). 경위·처방 = [`docs/report-from-pdf.md`](./docs/report-from-pdf.md) §3-A.
...
4. **이미지 — 영상 프레임 캡처(필수)**: §5 규칙 + 아래.
   ...
   - **영상을 480p로 한 번만 받아 두고 프레임을 뽑는다**: `yt-dlp -f "135/134/best[height<=480]" -o <id>.mp4 <url>` → `ffmpeg -ss <초> -i <id>.mp4 -vframes 1 -q:v 2 out.jpg`. 🔴 **구간 다운로드(`--download-sections`)는 2026-08-25부터 쓰지 말 것** — 유튜브가 통합 포맷 18을 내려 android_vr 클라이언트 URL만 남았고 ffmpeg가 거기서 403을 받아 **캡처가 한 장도 안 나온다**(오류 없이 멈춰 알아채기 어렵다). 8~20분 영상은 480p 전체가 30~60MB·수 초라 지점이 여러 개면 오히려 빠르다. 커밋된 `scripts/yt_report/capture.py`가 이미 이 방식이다.
   - **화질**: 480p(format 135)면 차트·도해·자막 판독에 충분하다. 받은 영상 파일은 작업이 끝나면 RUN_DIR째로 지운다(14편 ≈ 390MB).
   ...
   - 🔴 **더 흔한 실패는 「토킹헤드」가 아니라 「차오르는 중」이다** — 표는 분명히 보이는데 비교 대상 칸이 비어 있는 프레임(리포트 117·122). 규칙·검사기 = [`docs/report-from-youtube.md`](./docs/report-from-youtube.md) §2-A. **게시 전 `settle.py --check` 필수.**
...
- **본문 작성·프레임 선별·차트 발굴은 에이전트/서브에이전트 단계**로 남겨 둔 수동 고품질 경로다.
  🔴 **이 수동 경로(유튜브·PDF 모두)는 최신 Opus 로 쓴다 — 현재 `claude-opus-5`**(사용자 지시 2026-09-11).
  서브에이전트에 위임할 때도 그 모델로 띄운다.
- 🔴 **자동 경로는 손대지 않는다**(같은 지시): `collect_yt_report.py`(Haiku) · `youtube.service.ts`(Gemini 2.5 Flash) ·
  `CLAUDE_SUMMARY_MODEL`(Haiku)은 그대로다. "오퍼스를 쓰라"는 것은 **사람이 Claude Code 에서 직접 쓸 때**의 이야기다.
  완전 자동 버전은 §2-A 를 볼 것.
...
- [ ] **§3-B 외화 수치에 원화 환산 병기 — `python scripts/verify_report_krw.py --run <원고>` EXIT=0**. 환율은 작성 시점 하나로 통일하고 각주에 한 번만 적는다
- [ ] §4 렌더 규칙 준수 — 렌더 후 raw `**`=0, 백틱/단독`<br>`/단일`~` 점검
- [ ] **번역투 검사 `python scripts/verify_translationese.py --run <원고>` EXIT=0** — 영어 원문을 옮긴 글은 필수. 한 단어씩 치환한 문장(`cover`→「덮다」)은 렌더·표 검사에 안 걸린다(2026-09-15 리포트 136 실사고 5곳)
