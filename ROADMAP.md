...
- [x] **E2E 테스트 (Playwright) — 완료 2026-09-10.** 「열리는가」 `e2e_smoke.py` 역할 5종 ×
  ...
  - 🔴 `/management/production` 은 **조작 요소가 없어 검사 대상이 아니다**(버튼이 로그아웃뿐 —
    다시 조사하지 말 것).
- [ ] 성능 최적화 (Lighthouse 90+ 목표) — **2026-09-10 처음 실측했다**(그전까지 잰 적이 없다).
  ...
  | 데스크톱       | **87** | 0.7s | 290ms     | —      | —        | —   |
  - 🔴 **목표 「90+」가 어느 기준인지 정해야 한다** — 데스크톱은 **3점 차이**이고 휴대폰은 **36점 차이**다.
    사내 대시보드라 실사용은 데스크톱이지만, 문서에 기준이 안 적혀 있어 판단이 갈린다.
...
- [ ] Vercel 프로덕션 배포 — ⚠️ **「배포를 안 했다」는 뜻이 아니다.** 자동 배포가 켜져 있고
      (`vercel.json` 의 `ignoreCommand` 는 **백업 파일만 바뀐 커밋만** 건너뛴다) 프로덕션은
      2026-09-10 로그인·화면 실물로 응답을 확인했다. **E2E 는 끝났고**(위 칸) 남은 조건은
      **Lighthouse 목표 달성 하나**다 — 그 기준을 정해 넘기면 이 칸도 함께 닫힌다.
