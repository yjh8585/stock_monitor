...
> 🔴 **fnguide 교체 자체는 끝났다.** 아래 남은 체크박스 3개는 fnguide 가 아니라
> **2026-07-18 데이터 감사의 잔여**로, 이 Phase 에 얹혀 있을 뿐이다(각각 LOW·현 데이터 무해).
> 「fnguide 가 아직 진행 중」으로 읽지 말 것 — 이 오해 때문에 2026-09-11 에 한 번 다시 조사했다.
...
### 2026-07-18 데이터 감사의 잔여 3건 (2026-09-10 건져 올림)

`docs/data-audit-2026-07-18.md` 는 **역사 기록**이다(P1~P5 정정은 그 문서의 「✅ 정정 실행 결과」 절에 완료로 남아 있고,
인프라 수정은 `7d897f9` 로 커밋됐다). 🔴 **다만 아래 3건은 그 문서 안에 「미착수」로 섞여 있어 여기로 옮겼다** —
문서가 보관되면 함께 잊힌다.

- [ ] **세진 극단연도** — `_get_audit_rcpt` 연도매핑 잔여. 세진 데이터 자체는 FY21~23 정정 완료이고
      dry-run 도 정상이라 **현 데이터에는 문제가 없다**(그래서 당시 미착수). 연도매핑을 손볼 때 함께 본다.
- [ ] **제품군 역오류**(LOW) — 비자동차를 자동차로 오분류(LG전자 TV = 구동계 등). `product_category_map` 과
      무관한 **name-level** 문제라 map 확장으로는 안 고쳐진다. 🔴 오탐 경계가 좁아 규칙을 넓히면 정상 분류가 깨진다.
- [ ] **기존 products 의 map 미적용** — 마이그레이션 `20260718000001` 로 map 을 넓혔지만 **미래 수집분만** 정규화된다.
      기존 행은 2026-07-17 재분류 때 트리거가 이미 '기타'로 정규화해 **raw 가 소실**됐다. 되살리려면 재수집이 필요하다.
      **2026-09-11 실측: `'기타'` 가 1,860건으로 여전히 최다**(다음이 차체 586 · 전장 533 · 구동계 423) —
      상태가 그대로임을 확인했다. 재수집 없이는 안 줄어든다.
