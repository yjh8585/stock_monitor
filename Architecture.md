...
- **`/oem/competition`** (신규 2026-08-13, 같은 날 차트 중심으로 재구성 · 2026-08-14 사용자 피드백 반영) — **종합 스코어보드 + 차종별 섹션 10개**(`components/oem/competition/`). 소스 `lib/oem-competition/source.ts`, 적재는 월 1회 `collect_oem_model_outlook.py`(§10).
  ...
  - **차트 7종** — 판매 추이 멀티라인(툴팁 YoY) · 경쟁차종 순위 가로막대 · 점유율 화살촉 덤벨 · 경쟁 지형도 버블 · 소비자 평가 레이더 · 유통재고 막대 · NHTSA 리콜/불만(지표 토글). 순위·점유율·버블에는 **기준 버튼(최근 12개월 · YTD)** 이 붙고, 그 집계는 월별 구체화 뷰에서 `buildPeriods`가 재계산한다. 공용 토큰은 `shared.tsx`. 차트 규칙·색 팔레트 2종 → [`docs/chart-guide.md §3`](./docs/chart-guide.md).
