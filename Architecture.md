...
- **`/oem/competition`** (신규 2026-08-13, 같은 날 차트 중심으로 재구성 · 2026-08-14 사용자 피드백 반영) — **종합 스코어보드 + 차종별 섹션 10개**(`components/oem/competition/`). 소스 `lib/oem-competition/source.ts`, 적재는 월 1회 `collect_oem_model_outlook.py`(§10).
  - **스코어보드**(`CompetitionScoreboard`) — 10종 × 5항목(판매 증감·점유율·**유통재고(미국)**·**안전성(미국)**·소비자 평가) 신호등 표. **종합 등급은 별도 열이 아니라 행 전체의 음영**(`color-mix`로 카드 배경에 섞은 불투명 색 — sticky 첫 열이 반투명이면 글자가 비친다). 색만으로 정보를 주지 않도록 차종명 앞에 신호등 점을 함께 둔다. 행 클릭 시 해당 섹션으로 앵커 이동. 다중 시장 차종은 **가장 나쁜 시장**의 등급(`worstSignal` — 평균은 한 시장의 위험을 지운다).
  - **차종 섹션**(`ModelSection`) — 다중 시장(셀토스 3·아반떼 2·니로 2)은 **시장 탭**으로 가른다(시장마다 경쟁군이 완전히 다르다). 각 시장에 KPI 4타일(판매·점유율·유통재고에 **증감 병기**) → **종합 판단 근거**(`ModelNarrative`, 근거만 펼친 채 나머지는 접힘) → 차트 7종 순.
  - **차트 10종** — 판매 추이 멀티라인(툴팁 YoY) · 경쟁차종 순위 가로막대 · 점유율 전년/현재 가로 그룹막대 · **점유율 추이 라인(12개월 이동 누계)** · 유통재고 막대 · **유통재고 추이 라인** · 경쟁 지형도 버블 · NHTSA 리콜/불만(지표 토글) · 소비자 평가 레이더(차종별 패널 분리) · **신차 사이클 누적 가로막대**(세대 나이 vs 경쟁, `model_cycle` JSONB). 수준 카드와 추이 카드를 **좌우로 짝지어** 배치한다. 순위·점유율·버블에는 **기준 버튼(최근 12개월 · YTD)** 이 붙고, 그 집계는 월별 구체화 뷰에서 `buildPeriods`가 재계산한다. 공용 토큰은 `shared.tsx`. 차트 규칙·색 팔레트 2종 → [`docs/chart-guide.md §3`](./docs/chart-guide.md).
  ...
  - 🔴 **미국 전용 지표의 자격**(`usMetricsBasis`) — Cox 딜러 유통재고·NHTSA 는 미국 데이터다. 미국 시장 탭이면 그 시장의 사실(`native`)이지만, **글로벌 시장(포르쉐 911)에 붙은 값은 참고치**(`reference`)라 표시만 하고 **신호등 등급을 매기지 않는다**(사용자 결정 2026-08-14). 인도·한국·중국·유럽 탭에는 값 자체가 붙지 않는다. Cox 이상치 제외(= 업계 평균 2배 초과로 수치 미공개)는 "값 없음"이 아니라 **RED** 로 판정한다.
...
scripts/                  # Python 수집 + onboarding
  lib/                    #   공용 모듈 (db.py, accounts_map, fx, revalidate, bootstrap, fnguide_client, retry … 차종 경쟁 5종은 아래 표)
...
| `oem-competition/`                                                                                                                   | `/oem/competition` — `oem_model_outlook` v2 컬럼 매핑(`types.ts`) + 최신 `note_date` 1건 선별(`pickLatestPerModel`) + 기준 2종 재집계(`buildPeriods`) + Cox 이상치 판정(`buildBrandInventory`), 전부 vitest                               |
...
🔴 **`country = 'China'` 는 「중국에서 팔린 대수」가 아니라 중국 업체의 글로벌 출하(내수 + 수출)다**(2026-09-11 실측 — 2025년 34.35백만 = CAAM 총 판매 34.40백만과 일치, 그중 수출 7.098백만. China 의 BYD 4,602,436 = BYD **글로벌** 총 판매 4,602,400). 수출분이 도착 국가에도 잡히므로 **국가별 값의 단순 합계는 시장 규모가 아니다**(2025 전 국가 합 98.5백만 vs 실제 글로벌 91.9백만). 경위·처방 → [`docs/gotchas-data-collection.md`](./docs/gotchas-data-collection.md).
...
**새 사외비 테이블 5-step** (AGENTS.md 에서 이관 · 2026-09-09)

1. 마이그레이션에서 `ENABLE ROW LEVEL SECURITY` — **정책은 만들지 않는다**(default deny).
2. `generate_typescript_types` 로 `lib/database.types.ts` 갱신 (🔴 손으로 붙인 `TableRow`·`ViewRow` 를 덮지 말 것 → [`docs/gotchas-ci-deploy.md`](./docs/gotchas-ci-deploy.md) §7).
3. `lib/supabase/confidential.ts` 의 `CONFIDENTIAL_TABLES` 에 한 줄 추가 — **이 배열이 명단 정본이다.**
4. 업로드 API 는 `confidentialDb...upsert` + `revalidateTag`.
5. 페이지는 `'use cache'` + `cacheTag` + `confidentialDb...select`.
...
## 부록 C. 디렉터리 지도 (AGENTS.md에서 이관 · 2026-08-25)

> 이관 사유: AGENTS.md 가 44,384B 로 상한 39,000B 를 2주째 넘고 있었고, 이 절 하나가 20,029B(45%)였다.
> AGENTS.md 자신의 규칙 — _"구조 설명이 길어지면 Architecture.md로 옮기고 여기선 참조한다"_ — 을 따랐다.
>
> ⚠️ 이 절의 **약속(금지·반드시·말 것)은 AGENTS.md 「폴더별 약속」에 원문 발췌**돼 있다.
> 문구가 갈리면 **이쪽(원문)이 정본**이고, 발췌를 고칠 때는 여기도 함께 고친다.
