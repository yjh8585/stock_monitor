-- v3 패치: JLR(재규어 랜드로버) 표준명 변경 + 랜드로버 단독 별칭 추가
-- 사용자 결정: JLR은 재규어·랜드로버 합작사라 두 브랜드 모두 'JLR'로 통합 표시.
--
-- v3 본 마이그레이션 (20260522000001) 직후 적용. expand_customer_name 함수만 CREATE OR REPLACE.
-- 트리거(companies_normalize_customers)는 그대로 살아 새 함수 정의로 동작.
...
BEGIN
  ...
  RETURN CASE
    ...
    WHEN cleaned IN ('Mitsubishi', '미쓰비시', '미쯔비시', '미쯔비시자동차', 'Mitsubishi Motors', '미쓰비시자동차') THEN ARRAY['미쓰비시']
    ...
    WHEN cleaned IN ('Great Wall Motor', 'Great Wall Motors', 'GWM', '그레이트월모터', '그레이트월모터스') THEN ARRAY['그레이트월모터스']
    ...
    -- JLR (재규어·랜드로버 합작사) — 두 브랜드 모두 JLR 표준명으로 통합
    WHEN cleaned IN ('Jaguar Land Rover', 'JLR', '재규어랜드로버', '재규어 랜드로버',
                     'Jaguar', '재규어', 'Land Rover', 'Land-Rover', 'Landrover', '랜드로버',
                     'Range Rover', '레인지로버') THEN ARRAY['JLR']
...
-- 기존 '재규어 랜드로버'·'재규어'·'Jaguar' 행을 모두 'JLR'로 재정규화
...
FROM (
  ...
  WHERE jsonb_typeof(cc.customers) = 'array'
    AND EXISTS (
      SELECT 1 FROM jsonb_array_elements(cc.customers) elem
      WHERE elem->>'name' IN ('재규어 랜드로버', '재규어', 'Jaguar')
    )
) subq
