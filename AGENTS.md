...
- **도메인 폴더** (페이지·기능 단위, 각각 `source.ts`로 fetch+cache+mapping 격리. 페이지는 호출만):
  ...
  - `lib/stellantis-forecast/` — **경영관리 스텔란티스 탭**(`/management/stellantis`) 매출 전망. `source.ts`(공개 3종은 anon, **자사 매출은 `confidentialDb`**) + `aggregate.ts`(pure, `aggregate.test.ts` 58 tests). 3축 = 출하(`stellantis_shipments`) · 소매(MarkLines `oem_sales_model_country_month`) · 재고(`cox_brand_inventory`) + 자사 매출(`pnl_entries` customer=`'Stellantis NA'`, basis=`standalone` — 연결은 월별이 2025부터라 시차 표본 부족). 항등식 **출하 − 소매 = 딜러 재고 증감**. 상세 → `docs/superpowers/specs/2026-07-15-stellantis-forecast-design.md`
    - **⚠️ MarkLines는 캐나다가 한 달 늦게 들어온다** → 3개국이 다 찬 분기까지만 쓴다(`lastCompleteQuarter`). 그냥 합산하면 최신 분기 소매가 과소집계돼 **재고 축적을 과대평가**한다(이 페이지가 판정하려는 바로 그것이라 치명적).
    - **⚠️ 스코프 정합**: IR North America 세그먼트는 **마세라티 제외**(별도 세그먼트) → 소매도 `MASERATI_MODELS` 제외로 맞춘다. MarkLines가 페라리 `SF90 Stradale` 7대를 2020년 FCA에 잘못 붙여 놨으므로 같이 배제.
...
- `collect_stellantis_shipments.py` — **Stellantis IR 북미 출하(도매)** → `stellantis_shipments`. SEC EDGAR 6-K(`data.sec.gov/submissions/CIK0001605484.json`으로 발견, **UA 헤더 필수**). **stellantis.com은 Akamai가 curl/requests를 403 차단하므로 쓰지 않는다.** 지역별 절대값 표가 실린 실적 PR은 **Q1/H1/Q3/FY 4회만** → **Q2 = H1 − Q1, Q4 = FY − H1 − Q3** 차분 도출(`is_derived=true`, 천대 반올림 ±1,000대). **2026 이후에도 동일**(분기 'Estimated Shipments' 릴리스는 산문 증감뿐, 절대값 표 없음) → 최신 분기가 비어 있는 게 정상.
