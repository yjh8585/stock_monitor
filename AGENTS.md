...
| `/oem`               | OEM **"전체" 탭** — 글로벌 MarkLines 대시보드(5개 `oem_sales_group_*` 테이블) + OEM 모델 outlook. 탭 네비는 `app/oem/layout.tsx`에서 통합 관리. 6탭: 전체/Stellantis USA/KG모빌리티/현대차/기아/우즈베키스탄.                                                                                                                                                                                                                                                                                                                                                                  |
| `/oem/stellantis-na` | Stellantis NA (FCA US LLC) brand·차종별 분기 판매 (2021Q1~). 출처: prnewswire.com FCA US LLC publisher의 분기당 1개 보도자료 HTML `<table>` (`collect_stellantis_na_sales.py`, requests + BeautifulSoup, Playwright 불필요). 데이터: `stellantis_na_sales` (PK=period_type/year_period/brand/vehicle_model/region; brand 6=Jeep/Ram/Chrysler/Dodge/Fiat/Alfa Romeo, Maserati 별도 PR 미수집; brand='Total'/vehicle_model='Total'은 합계 row, region='US'; `vehicle_powertrain_map` LEFT JOIN). 분기 PR이 CYTD 컬럼 동봉이라 Q4의 경우 period_type='year'(연 누계) 한 세트 추가 적재 — Q1~Q3 연 누계는 분기 SUM으로 자연 도출. 차트: KPI 4종 + 분기 brand stacked(분기/연 토글, 합계 라벨) + 브랜드 mix(100% stacked) + PT mix(100% stacked) + 차종 TOP10(brand 1단계 드롭다운, region 미사용=단일 'US'). URL 매핑: `scripts/lib/stellantis_pr_urls.json` (22분기 영구 캐시) + `--auto-discover`로 publisher index에서 신규 분기 발견. HTML sha256 캐시는 `data/_stellantis_pr_cache/`. cross-check: brand_total vs 모델 SUM (Q ±25, YTD ±100, source-side 미세 누락 허용) + 회사 합계 ±5 (실패 시 abort). cron: 분기 첫 달 3일 03:00 UTC(`collect-stellantis-na-sales.yml`). |
| `/oem/kg-mobility`   | KG모빌리티 차종별 판매 (월별, 2021~). 출처: kg-mobility.com IR 엑셀(`collect_kg_mobility_sales.py` Playwright `expect_download()`). 데이터: `kg_mobility_sales` (region=내수/수출, `vehicle_powertrain_map` LEFT JOIN). 차트: KPI 4종 + 월별 시계열(YoY) + PT mix(100% stacked) + 차종 TOP10 + 내수/수출 분리. 매월 15일 03:00 UTC cron(`collect-kg-mobility-sales.yml`).                                                                                                                                                                                        |
| `/oem/hyundai`       | 현대차 차종별 판매 + 해외 공장별 + 지역별 수출 (월별, 2021~). 출처: hyundai.com IR 엑셀 3종(`button.btn-download` Playwright) + IR 사이트 hover API(연도별 9개 region 도매 합계) + **미국/유럽 현지(retail) 엑셀 2종** (`collect_hyundai_retail.py`, `hyundai_retail_sales` 테이블, 마이그레이션 `20260526000005`). 데이터: `hyundai_sales` (region/factory/vehicle_model PK; factory=""=국내, `vehicle_powertrain_map` LEFT JOIN) + `hyundai_export_regions` (한국 출하 세부 region 월별 + IR 도매 region 연 합계 — `source` 컬럼으로 구분) + `hyundai_retail_sales` (US/EU retail, US는 industry_total/market_share 동봉). 차트: KPI + 판매 추이(월/연 토글) + PT mix + 해외 공장별 stacked + TOP10(region 토글) + 지역별 수출/도매 region 분포. 도매 cron: 매월 15일 03:00 UTC(`collect-hyundai-sales.yml`), retail cron: 매월 16일 03:00 UTC(`collect-hyundai-retail.yml`). |
| `/oem/kia`           | 기아 차종별 판매 + 해외 공장 5종 + 지역별 수출 10종 (월별, 2021~). 출처: worldwide.kia.com IR 엑셀 3종(`collect_kia_sales.py` JSON API + Playwright APIRequestContext 다운로드: 차종별판매실적·해외공장판매실적·지역별수출실적). 데이터: `kia_sales` (region IN ('', '내수', '수출', 'CKD') + factory 5종 + `vehicle_powertrain_map` LEFT JOIN, **Aggregate 모델은 CKD section 합계 행이라 TOP10에서 제외**) + `kia_export_regions` (10 region × vehicle_type 8종을 6 카테고리로 normalize). 차트: KPI 4종 + 판매 추이(월/연 토글) + PT mix(EV 8종 매핑) + 해외 공장 5종 stacked + 지역별 수출 10종 stacked + 수출 차종 type mix(승용/RV/상용/특장/CKD 일반/CKD 특장) + 차종 TOP10(전체/국내/내수/수출). 매월 16일 03:00 UTC cron(`collect-kia-sales.yml`, 현대 15일과 1일 분산). 마이그레이션 `20260527000001`, `20260527000002`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `/oem/uzbekistan`    | 우즈베키스탄 자동차 시장 (PR6, 2024~). 출처: **uzavtosanoat.uz** (회사별 sales 매월 보도자료, RU 정규식 파싱 + YTD 차분 → 월별 row) + **uzavtosanoat.uz Statistical info** (연간 production by brand, Chevrolet/BYD/LCV/Engines 2016~2025). 데이터: `uzbekistan_auto_stats` (단일 통합 테이블, kind=sales\|production, period_type=month\|quarter\|year, company enum 6개=UzAuto Motors/Khorezm Auto/ADM Jizzakh/BYD Uzbekistan Factory/SamAuto/Asaka Motors, source_type=uzavtosanoat\|stat-uz). **UzAuto Motors만 companies 테이블 등록(UZMT, data_source=uzauto-pdf)**, 나머지 5개는 sales row의 company 컬럼만 사용. 차트: KPI 3장 + 회사별 sales 월/연 stacked + 연간 production by brand stacked. 마이그레이션 `20260527000004`. cron: 매월 20일 03:00 UTC(`collect-uzbekistan-sales.yml`, 보도자료 14~18일 발표 후 안전 일정). stat.uz 분기 production은 추후 별도 수집 추가 예정.            |
...
- 도메인 폴더 (페이지·기능 단위):
  ...
  - `lib/oem-companies/<slug>/` — OEM 회사별 탭 (`/oem/<slug>`) 데이터 입구. `<slug>` ∈ {kg-mobility, hyundai, kia, stellantis-na}. 패턴: `source.ts`('use cache' + cacheTag('oem-<slug>-sales') + vehicle_powertrain_map LEFT JOIN) + `aggregate.ts`(pure: YoY/YTD/TOP10/PT mix + 회사별 보강 함수) + `aggregate.test.ts`. 공통 컴포넌트는 `components/oem-companies/common/`, 회사별 보강은 `components/oem-companies/<slug>/`.
  - `lib/hansae/`, `lib/naver/`, `lib/sentiment/` — 페이지/기능별

### `scripts/` — Python 데이터 수집

prefix 컨벤션. 신규 스크립트는 같은 카테고리 prefix 사용.

- `collect_*.py` — 외부 → DB 수집. **PDF-only 회사**(UzAuto Motors)는 `collect_uzauto_financials.py`: `/investors` HTML 파싱(우즈벡어/영문 양쪽 정규식) → PDF sha256 캐시(`uzauto_pdf_cache`, 마이그레이션 `20260526000001`) → 변경분만 Anthropic API(`claude-opus-4-7`)에 PDF document + tool_use(submit_financials)로 구조화 추출 → financials upsert. 연도 오름차순 처리로 재진술(restatement) 정책 자연 보장. 플래그: `--reprocess-all`(캐시 무시), `--dry-run`(링크 추출만). 매주 월요일 03:00 UTC GHA cron. **현대차 분기 IR PDF**도 동일 패턴: `collect_hyundai_quarterly_earnings.py`는 `/quarterly-earnings` 페이지의 분기별 "실적 발표 자료" PDF를 Playwright(`expect_download`)로 받아 sha256 캐시(`hyundai_quarterly_earnings.pdf_sha256` 컬럼) → Anthropic API + `tool_use(submit_earnings)` → `hyundai_quarterly_earnings` upsert(PK=fiscal_year,fiscal_quarter). 매출/부문매출/영업이익/판매량/친환경 비중 일괄 추출. 플래그: `--year-from`/`--year-to`/`--quarter`/`--reprocess-all`/`--dry-run`. 마이그레이션 `20260526000006`.
...
- `scripts/_archive/` — 일회성 보관. git history 보존, 향후 비슷한 작업 시 패턴 참고용. README 참고.
...
대부분 GitHub Actions가 Python 직접 호출(로컬 `scripts/venv` 없이). `cron-sentiment`만 Vercel cron 대체용으로 curl 호출.

- 가격·환율: 매일/매시간
- 재무: 분기별 (1/4/7/10월 15일)
- 뉴스·감성: 4시간/일간
- DART·매크로·해운·철강·원자재: 일간/주간
- Vercel cron 대체(curl 트리거): `cron-sentiment` — Hobby 플랜 일 1회 제약 회피. secret 필요: `APP_BASE_URL`, `CRON_SECRET`. (한세 현재가·시총은 별도 cron 없이 `collect-hansae-intraday.yml`이 KIS로 5분 분봉 수집 시 `companies` 스냅샷도 함께 갱신.)
...
- OEM 회사별 차종 판매: `collect-kg-mobility-sales.yml`(PR2) + `collect-hyundai-sales.yml`(PR3) 매월 15일 03:00 UTC + **`collect-kia-sales.yml`(PR4) 매월 16일 03:00 UTC**(현대와 1일 분산, retail과 같은 날) + **`collect-stellantis-na-sales.yml`(PR5) 분기 첫 달 3일 03:00 UTC**(1·4·7·10). Playwright `expect_download()` 또는 APIRequestContext 사용 (KG/현대/기아 정적 엑셀 링크 없음). 현대는 엑셀 3종(차종별·해외 공장별·지역별 수출) 통합 적재 → `--kind {model,factory,export,all}` 옵션 + 같은 workflow의 step에서 `collect_hyundai_ir_summary.py`(API POST, Playwright 불필요)로 IR 사이트 9개 region 연 도매 합계도 함께 갱신(`hyundai_export_regions`, source=ir-summary). 기아는 worldwide.kia.com JSON API(`/api/investors/business-sales-results`) → 엑셀 3종(차종별·해외 공장·지역별 수출) Playwright APIRequestContext 다운로드. **Stellantis NA는 prnewswire.com FCA US LLC publisher의 분기당 1개 보도자료 HTML `<table>`을 requests + BeautifulSoup으로 파싱 (Playwright 불필요)** — URL은 `scripts/lib/stellantis_pr_urls.json`(22분기 영구 캐시) + `--auto-discover`로 publisher index에서 신규 분기 발견. HTML sha256 캐시(`data/_stellantis_pr_cache/`)로 변경분만 재처리. `workflow_dispatch`의 `year_from`/`year_to`/`quarter`/`reprocess_all`/`auto_discover` 입력. secret 필요: 기존 cron과 동일.
- 현대차 retail(미국/유럽 현지 판매): `collect-hyundai-retail.yml`(Phase 2C) — 매월 16일 03:00 UTC(도매보다 1일 뒤). 같은 IR 페이지의 "미국/유럽 현지 판매" 엑셀 2종(`button.btn-download` Playwright) → `hyundai_retail_sales`(US/EU). US는 industry_total/market_share 자동 적재. `workflow_dispatch`의 `year_from`/`year_to`/`region` 입력. secret 필요: 기존 cron과 동일.
- 현대차 분기 IR 보고서(매출/영업이익/판매량): `collect-hyundai-quarterly.yml`(Phase 2B) — 매월 25일 03:00 UTC. `/quarterly-earnings` 페이지의 "실적 발표 자료" PDF(분기당 1개, `q{1-4}-{YYYY}-...-ko.pdf`)를 Playwright로 다운로드 → PDF sha256 캐시 → 변경분만 Anthropic API(`claude-opus-4-7`) + tool_use(submit_earnings) → `hyundai_quarterly_earnings` upsert. PK=(fiscal_year, fiscal_quarter) → 재진술 자연 우선. `workflow_dispatch`의 `year_from`/`year_to`/`quarter`/`reprocess_all` 입력. secret 필요: 기존 cron과 동일 + `ANTHROPIC_API_KEY`.
...
│ hyundai.com IR │   │ collect_hyundai_*    │   │ hyundai_sales   │   │                   │
│ (PW expect_dl) │   │ (sales+retail)       │   │ hyundai_retail* │   │                   │
└────────────────┘   └──────────────────────┘   └─────────────────┘   └───────────────────┘
                              │                                              ▲
                  GitHub Actions 29개 워크플로                                │
                  (cron · 수동 dispatch)                                      │
                              │                                              │
                              ▼                                              │
