...
- **보호 라우트 UI Playwright 검증**: `.env.local` dotenv 로드(`MOBILITY_ID/PW`) + 클릭 후 `wait_for_url(lambda u: '/login' not in u)`(networkidle은 client redirect 전 반환). `LazyMount` 차트(recharts)는 IntersectionObserver라 `mouse.wheel`로 스크롤해야 마운트(`wait_for_selector` 데드락). Turbopack dev 첫 진입 시 `/management/*` 탭이 간헐 404(컴파일 캐시) → 새로고침/dev 재시작이면 정상(코드 회귀 아님 — 미수정 탭도 동일하면 환경 이슈).
