...
> 🔴 **단 판매의 「판매 시장」은 중국 브랜드에 대해선 사실이 아니다** — `country = 'China'` 는 내수 + 수출을 합한 글로벌 출하다(2026-09-11 실측) → [`gotchas-data-collection.md`](./gotchas-data-collection.md).
...
🔴 **2026-09-15 정정 — 「로컬엔 유효한 쿠키가 없다」는 더 이상 사실이 아니다.** `scripts/.env` 의
`MARKLINES_COOKIE` 에 **세션 쿠키 9종(`PLATFORM_SESSION`·`XSRF-TOKEN` 등)이 살아 있고**, 이것으로
분석 보고서 본문(`/en/report/<코드>`)까지 그대로 받힌다(실측: 62만 자·표 24개). 아래 우회 통로는
그 쿠키가 만료됐을 때의 대비책이다. 자세한 것은 바로 아래 「쿠키가 «두 곳»에 있다」.

(과거 사정) GitHub Secrets 는 write-only 라 값을 조회할 수 없고, 브라우저에서 다시 뽑는 길도 막혀 있다:
...
## 🔴 MarkLines 쿠키가 «두 곳»에 있고, 껍데기 쪽이 이기면 조용히 유료 벽을 받는다 (2026-09-15 실측)

같은 이름 `MARKLINES_COOKIE` 가 **두 파일에 다른 값으로** 들어 있다.

| 파일           | 들어 있는 쿠키                                           | 결과        |
| -------------- | -------------------------------------------------------- | ----------- |
| `scripts/.env` | **9종** (`PLATFORM_SESSION`·`XSRF-TOKEN`·`PLAY_LANG` 등) | 본문 전체   |
| `.env.local`   | **2종** (`_ga`·`_gid` — 구글 애널리틱스뿐)               | **유료 벽** |

**`.env.local` 쪽은 세션 쿠키가 아니라 껍데기다.** 그런데도 이름이 같아서, 로드 순서가 뒤집히면
아무 에러 없이 그 값이 쓰인다.

### 왜 조용한가 — 실패가 «성공처럼» 생겼다

껍데기 쿠키로 보고서를 받으면 이렇게 온다.

```
HTTP 200 · 342,360자 · <table> 0개
```

**200 이고 34만 자다.** 「받아졌다」로 읽으면 표가 통째로 없는 줄 모르고 그대로 쓴다. 본문 자리에는
`<div class="register-paid-box">This report is for paid members only.</div>` 가 들어 있고, 서문 몇
단락만 남는다 — 요약을 만들면 **그럴듯한 글이 나오므로 더 안 걸린다.**

### 판별법 — 1초

```python
wall = 'class="register-paid-box"' in html      # 유료 벽인가
print(html.count("<table"))                     # 정상이면 십 단위, 벽이면 0
```

🔴 **`<table>` 0개는 「표가 없는 보고서」가 아니라 거의 언제나 「로그인이 안 된 것」이다.**
MarkLines 분석 보고서는 표가 본체다(실측: 일본편 24개 · 유럽미주편 18개).

### 레포 스크립트는 안전하다 — 새 스크립트만 조심하면 된다

`scripts/lib/bootstrap.py` 의 `init_script()` 는 **`scripts/.env` 를 먼저** 읽고 `.env.local` 을
나중에 읽는데, `load_dotenv` 는 기본이 `override=False` 라 **먼저 읽은 쪽이 이긴다.** 그래서
`init_script()` 를 쓰는 한 항상 진짜 세션 쿠키가 잡힌다.

⚠️ **사고는 `init_script()` 를 안 쓰고 `load_dotenv` 를 직접 부를 때 난다.** 습관대로
`.env.local` 을 먼저 적으면 껍데기가 이긴다(이 문서를 쓰게 만든 실사고가 그것이다).
**임시 스크립트라도 `init_script(__file__)` 를 쓸 것** — 이것이 유일한 예방책이다.

### 곁가지 — 옵시디언 클리핑은 HTML 속성에 실린 데이터를 잃는다

같은 보고서를 웹 클리퍼로 받아 두면 셀의 글자는 살아도 `bgcolor` 같은 **속성에 실린 값이 사라져**
빈칸이 된다(2026-09-15 파워스티어링편 실사고 — 「조사 중」을 「공급 없음」으로 읽을 뻔했다).
범례 행도 함께 날아간다. **표가 중요한 보고서는 클리핑 말고 쿠키로 원문을 받을 것.**
