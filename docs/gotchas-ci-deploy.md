...
### 6-B. dev 서버를 «파이프에 물려» 띄우면 조용히 죽는다 (2026-09-18 실측)

백그라운드로 dev 서버를 올리면서 로그를 줄이려고 `npm run dev 2>&1 | head -40` 처럼
**파이프를 물리면 안 된다.** `head` 가 40줄을 받고 파이프를 닫는 순간 `npm` 이 SIGPIPE 로
끝나고 **dev 서버가 함께 내려간다.**

🔴 **터지는 시점이 뜨는 시점과 떨어져 있어 오진하기 쉽다.** 처음 몇 분은 멀쩡히 응답하다가
로그가 40줄에 도달한 «나중에» 죽는다. 그 뒤의 Playwright 검증은 `Timeout 30000ms exceeded`
로 실패하는데, 이걸 **화면 결함이나 컴파일 지연으로 읽고 코드를 고치기 시작하면**
멀쩡한 것을 망가뜨린다.

- **그냥 `npm run dev` 만** 백그라운드로 돌린다(하네스가 출력을 파일로 받아 준다).
  기동 확인은 로그 파일을 `grep` 하거나 `curl /login` 이 200 을 낼 때까지 기다린다.
- 죽고 나면 **잠금만 남아** 재기동이 `Another next dev server is already running` +
  PID 안내로 거부된다. 그 PID 를 `taskkill //PID <pid> //F` 로 정리한 뒤 다시 띄운다.
- 🔴 **`TaskStop` 으로 백그라운드 작업을 멈춰도 next dev 프로세스는 살아남는다**(떨어져 나간다).
  포트 소유자를 직접 찾아 끝낸다 — `netstat -ano | grep ":3001 " | grep LISTENING`.
...
## 11. 설계 문서에 자격증명을 평문으로 적었고, 과거 기록을 지워도 GitHub 에는 남았다 (2026-09-09)

**증상**: E2E 확장 뒤 「자격증명이 샜나」를 확인하다 별건을 찾았다.
`docs/superpowers/specs/2026-05-28-admin-role-permissions-design.md` 11행에
관리자 아이디·비밀번호가 **값 그대로** 적혀 있었다. 넉 달간 아무도 몰랐고
**이 저장소는 공개(PUBLIC)** 라 인터넷에 그대로 노출돼 있었다.

**원인**: 대화에 나온 값을 설계 문서에 그대로 옮겨 적었다. 악의가 아니라
...
3. `git filter-repo --replace-text` 로 649 커밋의 과거 기록을 다시 쓰고
   `--force-with-lease` 로 원격을 덮었다. 로컬·번들 백업에서 값이 0건임을 확인했다.

🔴 **그런데 3번은 끝이 아니다 — GitHub 에서 옛 커밋이 SHA 로 «지금도» 열린다.**
덮어쓴 직후 실측했다: 유출 커밋 `6e0ab0f` 를 API 로 부르면 그대로 응답하고,
그 시점 파일 본문에서 비밀번호가 **읽힌다.** GitHub 은 참조가 끊긴 객체를 자체
정리(GC) 전까지 보관하고, 그 정리를 사용자가 직접 시킬 방법이 없다
(필요하면 GitHub 지원에 요청해야 한다).

⚠️ **그래서 순서가 뒤집히면 안 된다 — 「자격증명 교체」가 먼저고 기록 정리는 나중이다.**
기록을 지우는 작업은 눈에 보이는 성과가 커서 먼저 하고 싶어지지만, 이미 공개된 값을
쓸모없게 만드는 유일한 조치는 **값을 바꾸는 것**이다. 기록 정리는 「앞으로 새로 보는
사람」을 줄일 뿐이다.

### 검사기를 만들 때 이 자리에서 두 번 고장 났다

같은 날 만든 `verify_no_secrets.py` 자신이 두 번 틀렸고, 둘 다 **초록이었다**:

1. **1판 — 짧은 값을 통째로 건너뛰었다.** 아이디 값이 `guest` 처럼 흔하면 온 파일에
   걸려 오탐 44건이 나기에 길이 하한을 뒀는데, 그 하한에 **정작 유출된 `ADMIN_PW`(5자)가
   걸려 검사 밖으로 빠졌다.** 이 검사기가 만들어진 바로 그 사고를 못 잡는 상태였다.
   🔴 「위반 0건」이 「대상에서 빠진 것」이었던 전형이다.
2. **2판 — 「같은 줄에 키 이름이 있으면 유출」로 완화했더니** `process.env.HMOBILITY_ID`
   같은 **환경변수를 읽는 정상 코드**가 걸렸다(오탐 4건). 값이 키 **뒤** 에 붙어야 유출이고
   앞에 있으면 참조하는 코드다.

지금은 값 길이에 따라 찾는 법을 나눈다(긴 값=전문 검색 · 짧은 값=`키=값` 꼴만) 하고,
두 오탐 형태를 **자기시험에 못 박았다**(`--self-test` 7/7).
⚠️ 처음엔 파일 1,792개 × 값 29개를 줄 단위로 돌려 3분을 넘겼다 — 파일 전체를 한 번
훑어 후보를 거르는 단계를 앞에 두어 16초가 됐다.

🔴 **검사기를 설명하면서 검사 대상을 만들지 말 것.** 첫 판 docstring 예시에 진짜 아이디를
적었고, 과거 기록 정리 때 그 줄이 함께 치환되면서 드러났다. 예시는 늘 가짜 값으로 적는다.
