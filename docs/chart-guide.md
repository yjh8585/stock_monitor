...
한 차종의 한 시장(`CompetitionMarket`)을 props 로 받는 차트 10종 + 요약 조각들.
**배치 규칙: 같은 질문에 답하는 두 카드를 이웃에** 둔다(2열 그리드라 좌우로 붙는다) — 판매 추이↔순위 ·
점유율 변화↔점유율 추이 · 재고 최신↔재고 추이. "지금 얼마인가"(수준)와 "어디로 가는가"(방향)를 나란히
봐야 한 쪽만 보고 오판하지 않는다(160일이라도 200일에서 내려오는 중인지 120일에서 오르는 중인지). **공용 토큰은 `shared.tsx`** — 대상 차종은 항상 `TARGET_COLOR`(파랑). 경쟁은 **차트 성격에 따라 두 팔레트로 갈린다**: 대상만 도드라지면 되는 막대류는 `rivalColor(i)`(회색 계열), 선이 얽혀 경쟁끼리도 구별돼야 하는 라인·레이더는 `rivalDistinctColor(i)`(고유색). 카드 껍데기는 `ChartCard`(제목 오른쪽 `actions` 슬롯 = 기준·지표 버튼), 빈 데이터는 `EmptyChart`.

| 컴포넌트                            | 유형                                                               |
| ----------------------------------- | ------------------------------------------------------------------ |
| `SalesTrendChart`                   | 다중 라인(대상 + 상위 3 경쟁, 24개월 월별) · 툴팁에 전년 동월 대비 |
| `ShareTrendChart`                   | 다중 라인 · **점유율 12개월 이동 누계** 궤적                       |
| `InventoryTrendChart`               | 다중 라인 · 브랜드 재고일수 월별 + 업계 관행선                     |
| `ModelCycleChart`                   | 가로 **누적** 막대(세대 나이 = 개선 전 + 개선 후) · 노후도 비교    |
| `CompetitorRankChart`               | 가로 막대 + `Cell` 개별 색 + YoY 라벨 · 기준 버튼                  |
| `ShareChangeBars`                   | 가로 그룹 막대(전년 동기 옅은 톤 / 현재 진한 톤) · 기준 버튼       |
| `PositionBubble`                    | 산점/버블(`ScatterChart`+`ZAxis`) · 기준 버튼                      |
| `ConsumerRadar`                     | 레이더 small multiples(차종당 1개, `domain=[0,5]` 고정)            |
...
- 🔴 **`ConsumerRadar`는 차종을 겹쳐 그리지 않는다** — 5축 × 4종 = 선 20개가 좁은 면에 얹혀 서로를 가린다. 고유색으로 바꿔도 해결되지 않았다(2026-08-14 두 번 실패). **패널을 쪼개되 모든 패널에 같은 회색 점선(비교군 평균)을 깔아** 비교 기준을 남긴다 — small multiples 는 **축척이 모든 패널에서 같아야** 성립하므로 `domain=[0,5]`·같은 `height` 를 반드시 공유한다(실측 검증: 5점 링 172px, 패널 간 차이 0px). 기준 평균은 **범례로 숨긴 차종까지 포함해** 낸다(표시 여부에 따라 기준이 흔들리면 패널 비교가 무너진다).
- 🔴 **차종 라벨은 `displayModel()` 로 만든다**(`shortModel()` 직접 호출 금지) — 브랜드를 앞에 붙이고, 이름에 이미 브랜드가 든 표기('Honda Pilot'·'VW Atlas')는 중복을 없앤다. 브랜드 표는 `market.modelBrands`. ⚠️ 브랜드가 붙으면 이름이 길어지니 **y축 폭·잘림 한도를 같이 올린다**(2026-08-14: 186px / 24자).
- 🔴 **단, 값이 _브랜드 단위_ 인 차트에는 `brandLedLabel()`** — "Toyota (Grand Highlander)" 처럼 브랜드를 앞세우고 차종을 괄호로 내린다. 지금은 Cox 유통재고 2종(`InventoryChart`·`InventoryTrendChart`)이 해당한다. 값은 브랜드 것인데 라벨이 `displayModel`("Toyota Grand Highlander")이면 **그 차종만의 재고로 읽힌다**(사용자 지적 2026-08-20: _"그랜드체로키가 아니라 지프 브랜드 전체네?"_ — 실은 대상·경쟁 4개 전부 브랜드 값이었고 라벨만 두 벌이었다). 부제에 단위를 적어 두는 것으로는 **막대 옆 라벨을 이기지 못한다.** 대상 차종명은 `outlook.modelName` 을 prop 으로 받아 괄호에 넣는다.
- 🔴 **`ModelCycleChart` 는 연식을 하나만 쓰면 정반대 결론이 난다.** 실측(2026-08-14): 그랜드체로키는 2021년 세대에 2026년 페이스리프트를 받아 "개선 이후 경과"가 0년이라 경쟁(2년차)보다 **더 신선해** 보이고, 반대로 "세대 나이"만 보면 2020년 세대인 Explorer 가 가장 늙었는데 정작 2024년에 손을 봤다. → **누적 막대로 둘을 한 그림에**(전체=세대 나이, 진한 오른쪽=개선 후 경과). 기준 연도는 브라우저의 오늘이 아니라 **`note_date` 의 연도**를 쓴다(옛 판정을 오늘 기준으로 재계산하면 없는 신선도가 생긴다).
- 🔴 **추이 2종의 결측 처리는 정반대다.** `ShareTrendChart` 는 **없는 달을 0 으로 센다**(경쟁차가 안 팔린 달은 행이 아예 없어서, 완전한 창만 인정하면 **모든 선이 통째로 빈다** — 2026-08-14 실측). 창이 덜 찼는지는 **데이터 시작월**로 따로 판정한다. 반대로 `InventoryTrendChart` 는 **null 을 절대 0 으로 바꾸지 않고 선을 끊는다**(Cox 가 감춘 값은 "0일"이 아니라 "너무 높음"이다).
- 🔴 **`ShareChangeBars`는 덤벨이 아니라 막대다** — 덤벨(점 2개)·화살촉 덤벨을 차례로 시도했으나 둘 다 "얼마나 큰가"가 전달되지 않았다(사용자 지적 2회). 막대는 원점에서 뻗어 **길이 = 값**이고 두 막대의 길이 차가 곧 변화량이다. 과거는 **같은 색의 옅은 톤 + 윤곽선**(§5-A 투명도 변형 문법).
...
- `ShareChangeBars`의 전년 점유율은 재집계 경로에서는 **실측값**이고, 월별 뷰가 없어 저장 스냅샷으로 떨어질 때만 **YoY 역산**이다. 부제가 그 사실을 구분해 밝힌다.
...
- 🔴 **`style={DATA_LABEL_STYLE}` 과 `fill=` 속성을 같이 쓰면 색이 안 먹는다.** 상수 안에 `fill: 'var(--foreground)'` 가 들어 있고 **CSS가 SVG 속성을 이긴다** — 증감을 초록/빨강으로 칠했는데 전부 검정으로 나갔다(2026-08-14). 색을 바꾸려면 `style={{ ...DATA_LABEL_STYLE, fill: 색 }}` 로 **style 안에서** 덮는다.
- **`LegendRow` 의 `color` 는 견본뿐 아니라 그 항목의 _글자색_ 이다.** "옅은 막대"를 알리려고 옅은 회색을 넣었더니 **라벨 자체가 안 읽혔다.** 농도·모양 같은 정보는 라벨 텍스트로 적고 색은 읽히는 값을 준다.
- 🔴 **값이 0 인 막대는 recharts 가 통째로 건너뛰고 남은 `LabelList` 의 `index` 를 당겨 쓴다** → 라벨이 한 칸씩 밀려 **엉뚱한 행에 붙는다**(2026-08-14: Traverse 자리에 그랜드체로키의 "5년차"). 값이 0 일 수 있는 시리즈에 라벨을 직접 붙이지 말고 **투명 앵커 막대**(§4-D `TOTAL_LABEL_ANCHOR`)에 붙인다 — 앵커는 항상 0 보다 커서 모든 행이 렌더되고 index 가 1:1 로 유지된다.
- 🔴 **`'use cache'` 페이로드에 필드를 더하면 옛 캐시 항목이 남아 `undefined` 로 들어온다.** 타입은 필수 필드라 통과하는데, 순회에서 터져 **카드가 아니라 화면 전체**가 클라이언트 렌더로 떨어졌다(2026-08-14). dev 는 `.next/cache` 삭제 + 재시작으로 풀린다(태그 revalidate 만으로는 안 풀렸다). 새 배열 필드는 소비하는 쪽에서 **`?? []`** 로 받는다.
- **`XAxis domain` 만 주면 눈금이 상한을 배수로 나누지 않는다** — 상한 30에서 `0·8·16·24·30` 이 나와 마지막 칸만 좁았다. 상한 후보와 **눈금 간격을 짝지어** 두고 `ticks` 를 직접 넘긴다.
