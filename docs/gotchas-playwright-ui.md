...
| 표 머리글을 눌러도 정렬이 안 됨                                        | 정렬은 「URL 이 바뀌는가」가 아니라 「목록이 바뀌는가」로 잰다 |
| 팝업·모달이 「안 열렸다」고 나옴                                       | `role="dialog"` 만으로 세지 말 것                              |
...
- 🔴 **Lighthouse 를 그냥 돌리면 「로그인 화면」을 잰다**(2026-09-10 실측). 이 사이트는 `/login` 말고 전부 세션이 필요해서, 프로덕션 주소를 그대로 주면 `/login` 으로 튕긴 화면이 측정된다 — **점수는 나오는데 대상이 틀린** 가장 나쁜 실패다. Playwright 로 로그인해 쿠키를 뽑아 `--extra-headers` 로 넘기고, 리포트의 `finalDisplayedUrl` 이 **의도한 경로인지 반드시 확인**한다. ⚠️ `CHROME_PATH` 만 주면 `Unable to connect to Chrome` 이 난다 → 크롬을 `--headless=new --remote-debugging-port=9222` 로 먼저 띄우고 `--port=9222` 로 붙인다. ⚠️ 무거운 화면은 `PROTOCOL_TIMEOUT` 으로 한 번 실패할 수 있다(부품사 TOP100) — **다시 돌리면 정상**이라 화면 결함으로 읽지 말 것.

- 🔴 **성능이 나쁘면 「표가 커서」로 잇지 말 것 — 실측이 그 가설을 기각했다**(2026-09-10). 화면 요소가 **40배** 차이인데 점수는 거의 같았다: 조직도 172개/55점 · 관련주식 1,544개/54점 · 부품사 TOP100 6,832개/58점. 전송량도 506KB 로 가볍다. 범인은 **모든 화면이 공통으로 지는 하이드레이션**이고, 가장 오래 도는 224KB 청크는 차트 같은 「뺄 수 있는 것」이 아니라 **React 런타임**이다. 🔴 **기준도 함께 적어야 한다** — 같은 화면이 휴대폰 기본값 **54점**, 데스크톱 **87점**이다(33점 차이). 「90+ 목표」만 적힌 문서는 **어느 기준인지 몰라 판단이 갈린다.**
...
## 🔴 정렬은 「URL 이 바뀌는가」가 아니라 「목록이 바뀌는가」로 잰다 (2026-09-11)

`/humanoid/reports` 에서 「작성일」·「게시일」 머리글을 눌러도 아무 일도 일어나지 않았다.
`PostList` 는 재사용했는데 **페이지가 정렬 `searchParams` 를 안 받았다** — `searchParams`
타입이 `{ page?: string }` 뿐이었고, 조회 함수와 `<PostList>` 둘 다 `'source_published_at'`·
`'desc'` 가 **박혀** 있었다. 그래서 링크는 정상적으로 만들어지고 **주소도 바뀌는데** 목록만
그대로였다.

🔴 **이 결함은 lint·타입·테스트 어디에도 안 걸린다.** `sort="source_published_at"` 는
타입상 완벽히 유효하고, 공용 컴포넌트를 「재사용했으니」 통과로 보인다. AGENTS.md 가 경고한
**「반만 쓰는 변형」**(인자 하나를 빠뜨려 기능이 통째로 죽는 것)의 전형이다.

- **검증 방법**: 머리글을 누른 뒤 **표의 날짜 열을 실제로 읽어 순서가 바뀌었는지** 본다.
  주소만 보면 언제나 통과한다(이 버그가 바로 그 상태였다).
- **기계 가드**: `scripts/verify_report_sort_wiring.py`(exit 0 정상). `PostList` 를 쓰는
  `app/**/page.tsx` 가 `sort`·`order` 를 searchParams 로 받고 `normalizeSort`/`normalizeOrder`
  (정본 `lib/reports/sort.ts`)를 쓰는지, 값을 박지 않았는지 본다. **양방향 시험 완료** —
  값을 도로 박으면 exit 1, 고치면 exit 0.
- ⚠️ **열 이름과 정렬 키가 직관과 반대로 보인다** — 이 표에서 「작성일」 = `source_published_at`
  (원문 발행일) · 「게시일」 = `created_at`(우리 사이트 게시일)이다. 조사하다 이걸 뒤집어
  읽으면 멀쩡한 정렬을 「뒤바뀌었다」고 오진한다(실제로 한 번 그랬다).

## 🔴 `role="dialog"` 만으로 세면 멀쩡한 팝업이 「안 열렸다」로 찍힌다 (2026-09-18 실측)

그림 확대 팝업을 검증하면서 `page.locator('[role="dialog"][aria-modal="true"]').count() == 1`
로 판정했더니 **열림도 False, 닫힘도 False** 가 나왔다. 두 판정이 동시에 거짓인 것이
단서였다 — 그 화면에는 **다른 dialog 가 이미 하나 더** 있어서 열리면 2, 닫히면 1 이었다.

🔴 **팝업은 실제로 잘 열리고 있었다.** 그대로 믿었다면 멀쩡한 컴포넌트를 고치기 시작했을 것이다.
(재시도 루프를 넣고서야 _"subtree intercepts pointer events"_ 로그에 팝업이 이미 떠 있는 것이
드러났다 — 클릭이 팝업에 막힌 것이다.)

- **판정은 내가 만든 그 팝업만 집어내는 셀렉터로** 한다 — `aria-label` 접두
  (`[role="dialog"][aria-label^="그림 확대"]`)처럼 고유한 표식을 쓴다.
- 🔴 **`== 1` 로 세지 말고 「내 것이 있는가」를 본다.** 페이지에 dialog 가 몇 개 더 있는지는
  화면마다 다르고, 나중에 늘어도 검사기는 말없이 뒤집힌다.
- 같은 이유로 `article img` 같은 넓은 셀렉터의 `.first` 도 위험하다 — 무엇이 잡혔는지
  `class`·`src` 를 한 번 찍어 확인하고 넘어간다(이번에도 그 출력이 `cursor-zoom-in` 을
  보여 줘서 「연결은 됐고 판정이 틀렸다」로 방향을 잡을 수 있었다).

## 🔴 휴머노이드 탭은 본문 여백을 «스스로» 챙겨야 한다 (2026-09-11)

`/humanoid/layout.tsx` 는 `flex-1 overflow-auto` 뿐이라 좌우 패딩이 없다. 게시판
`/reports` 는 `app/reports/layout.tsx` 의 `mx-auto max-w-7xl px-12 sm:px-20 lg:px-24` 가
여백을 주는데, 휴머노이드 탭은 그 컨테이너가 없어 각 페이지의 `p-6` 만 남았다.

실측(1440px · 사이드바 제외): **게시판 97px vs 휴머노이드 25px** — 표가 가장자리에 붙는다.
→ `app/humanoid/reports/layout.tsx` 를 게시판과 **같은 클래스**로 신설해 맞췄다(재측정 97px).
같은 글을 양쪽에서 볼 수 있으므로 폭이 달라 보이면 안 된다.

⚠️ `/humanoid/research`(증권사 리포트)는 여전히 25px 이다 — 표를 `rounded-md border` 카드로
감싸고 자체 필터 바를 둔 **다른 레이아웃**이라 이번에 건드리지 않았다.

## 컴포넌트 함정 (AGENTS.md에서 이관, 2026-08-12)

### 🔴 `bg-card`(순백)와 `bg-background`(연녹색)는 이 테마에서 «다른 색»이다 (2026-09-18 실측)

`app/globals.css` 의 `--background` 는 `oklch(0.9789 0.0082 121.6272)` — **색상각 121° 의
아주 연한 녹색**이다. 반면 `--card` 는 `oklch(1 0 0)` 순백이다. 이름만 보면 둘 다 「바탕」
이라 아무거나 써도 될 듯하지만, **표를 나란히 놓으면 한쪽만 희게 뜬다.**

**실사고**: 손익 「6. 부문별 매출 비중」을 1번 「전사 비용구조」 양식으로 만들면서 라벨 셀에
`bg-card` 를 그대로 가져왔다. 그런데 바로 위 5번 「부문 실적」(`PnlTable`)의 부문·연도
sticky 열은 `bg-background` 다 → **사용자가 두 표를 나란히 보고 지적해서야** 알았다.

- 🔴 **lint·타입·테스트 어디에도 안 걸린다.** 두 클래스 다 유효하고, 표 자체는 멀쩡히 렌더된다.
- **새 표의 sticky 라벨 셀은 옆 표가 무엇을 쓰는지 보고 맞춘다** — 손익 표들은 `bg-background`,
  1번 전사 비용구조만 `bg-card` 다.
- **눈으로 가르려 하지 말 것.** 두 색의 차이가 미세해 캡처를 봐도 헷갈린다. Playwright 로
  **계산된 값을 읽어 대조**한다 — 맞으면 두 셀이 같은 문자열을 낸다
  (`el => getComputedStyle(el).backgroundColor` → `lab(97.6425 -1.47271 2.65141)`).
