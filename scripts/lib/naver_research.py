...
# 로봇 밸류체인이라 추적하지만 **본업이 따로 있는** 종목. 이들만 추적 목록에 있어도
# 제목 핵심어를 «함께» 요구한다(사용자 결정 2026-09-14).
#
# 🔴 왜 필요한가: 「추적 종목이면 제목과 무관하게 통과」가 이 셋에서는 **일반 자동차 실적
#    리포트를 통째로 끌어왔다.** 2026-09-14 실측 — 6개월 소급 적재 59건 중 **36건**이 이 셋이었고
#    제목에 로봇 낱말이 하나도 없었다("2Q26 Review: 모난 것이 전혀 없다" ·
#    "2010년 이후 첫 A/S 영업이익률 27%대 회복" · "전장 믹스로 원가 역풍을 흡수").
#    셋 다 리포트가 많은 대형주라 **숫자를 지배한다**(보유 31·24·19건).
#    2026-08-25 사용자 지시 「관련성 떨어지는 거 제거하고 필요한 것만」의 취지와도 어긋났다.
#
# 🔴 **로봇이 본업인 종목은 여기 넣지 않는다** — 두산로보틱스·로보티즈·에스피지·에스비비테크는
#    제목에 로봇 낱말이 없어도("업종 내 차별적 매출/수익 성장성 보유") 로봇 리포트다.
#    가르는 기준은 시가총액이 아니라 **「이 회사 리포트의 대부분이 로봇 이야기인가」**이다.
DIVERSIFIED_TICKERS = frozenset(
    {
        "005380",  # 현대차
        "012330",  # 현대모비스
        "204320",  # HL만도(에이치엘만도)
...
def is_relevant(
    ...
    """이 리포트를 **저장하고 정리할 것인가** (사용자 지시·승인 2026-08-25).
        ...
        버린다   ← 본업이 따로 있는 추적사(DIVERSIFIED_TICKERS)인데 제목에 로봇 핵심어가 없으면
        남긴다   ← 종목분석이면서 (우리 추적 종목 OR 제목에 로봇 핵심어)
        남긴다   ← 산업분석이면서 제목에 로봇 핵심어
        그 외    ← 버린다

    🔴 `ticker` 를 넘기지 않으면 ③ 조항이 **동작하지 않는다**(옛 동작). 호출부 셋은 전부
       ticker 를 갖고 있으므로 반드시 넘긴다 — 빠뜨려도 에러가 없고 조용히 옛 규칙이 된다.

    🔴 판정이 세 번 바뀌었다. 되돌리지 않도록 경위를 남긴다.
    ...
    ③ ①의 「추적 종목이면 제목 무관 통과」가 **현대차·현대모비스·HL만도에서 역효과**를 냈다.
       셋은 로봇 밸류체인이지만 본업이 자동차라, 분기 실적 리뷰가 전부 딸려 들어와 숫자를
       지배했다(2026-09-14 실측 — 6개월 소급 59건 중 36건). 사용자 지적으로 **그 셋만**
       제목 핵심어를 함께 요구하게 좁혔다. 명단·근거 = `DIVERSIFIED_TICKERS`.
    ...
    if kind == KIND_COMPANY:
        # ③ 본업이 따로 있는 추적사는 제목이 로봇 이야기일 때만 남긴다.
        if is_tracked_company and ticker in DIVERSIFIED_TICKERS:
