...
`model_cycle` (신차 사이클 비교 차트 입력) 채점 규칙:
- **입력에 나온 시장마다 하나씩** 블록을 만들고, 각 블록에 **대상 차종 1개(`is_target`=true) +
  그 시장 "경쟁 차종 동기간 판매" 목록의 상위 3개**를 넣는다(consumer_scores 와 같은 구성).
- `model` 은 **입력 데이터에 쓰인 표기 그대로** 옮긴다(임의로 바꾸면 화면이 매칭에 실패한다).
- `last_full_change` = 현행 세대가 **처음 나온 연식**(완전변경/풀체인지). 4자리 연도.
- `last_update` = 마지막으로 **상품성이 개선된 연식**(페이스리프트·대규모 연식변경).
  완전변경 이후 개선이 없었으면 `last_full_change` 와 **같은 값**을 넣는다.
- 🔴 **이 두 값은 대상 차종만이 아니라 경쟁 차종에도 반드시 채운다.** 이 표의 목적은
  "대상이 경쟁 대비 노후한가"를 판정하는 것이고, 경쟁 연식이 비면 판정 자체가 불가능하다.
- 웹 검색 결과에 다음 세대·페이스리프트 소식이 있으면 `next_event_type`·`next_event_timing`
  에 담는다. **근거가 없으면 "미정" + 빈 문자열**로 둔다 — 지어내지 않는다.
- 연도가 불확실하면 가장 널리 알려진 값을 쓰되, `note` 에 그 사실을 한 줄로 밝힌다.
"""
