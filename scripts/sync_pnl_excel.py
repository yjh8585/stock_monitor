...
처리 흐름
--------
1. openpyxl로 엑셀 로드 (data_only=True, read_only=True)
...
EXCEL_PATH = Path(__file__).resolve().parents[1] / '참고' / '손익' / '자료정리_월별손익.xlsx'
TABLE = 'pnl_entries'
...
def summarize(entries: Iterable[dict[str, Any]]) -> None:
  """dry-run 요약 출력 (기준×연도×월수)."""
  from collections import defaultdict
  agg: dict[tuple[str, int], dict[str, Any]] = defaultdict(
    lambda: {'rows': 0, 'months': set(), 'revenue_sum': 0.0}
  )
  for e in entries:
    ...
    if e.get('revenue') is not None:
      agg[key]['revenue_sum'] += float(e['revenue'])

  logger.info('--- dry-run 요약 (basis × period_year) ---')
  for key in sorted(agg.keys()):
    v = agg[key]
    months = sorted(v['months'])
    logger.info(
      f'  {key[0]:<13} {key[1]} | rows={v["rows"]:>5} | '
      f'months={months} | revenue_sum={v["revenue_sum"]:,.0f}'
    )
