...
#    2026-09-10 재정리: 「검증 명령」 절의 명령 블록·실행 절차를 `docs/commands.md` 로 이관하고
#    약속 3건만 남겼다(agents 레포가 2026-08-25 에 같은 방식으로 `docs/commands.md` 를 가른 선례).
#    37,465 → 34,082B. 🔴 **여유가 35B 였던 것이 문제의 본질이었다** — 상한은 물고 있었으나
#    남은 자리가 없어 한 줄만 더해도 터지는 상태였고, 그래서 다이어트가 아니라 인상 압력이 생긴다.
AUTOLOAD_WARN_BYTES = 36_600   # 실측 34,082 + 여유 2,500. 넘으면 참조형을 Architecture.md·docs/ 로 옮길 시점
